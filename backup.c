...
int check_validation(int cmd){
	...
	switch(span){
		case UP:		
			...
			if(map[target_y][target_x] == EMPTY) return 1; //empty
			if(map[target_y][target_x] > BASE) return 1; //base
				...
				if((map[item_target_y][item_target_x] > BASE) && ((map[item_target_y][item_target_x]/10-1) == user_idx)) return (0 - map[target_y][target_x]);
